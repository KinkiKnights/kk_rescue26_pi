...
#    3. 2GB スワップ領域の作成
#    4. ubuntu-desktop / ros-jazzy-desktop と関連ツールの導入
#    5. kk_rescue26_pi 各コンポーネントの依存パッケージ導入
#       (USB WiFi ドングル RTL8811AU の DKMS ドライバは既定で無効。
#        有効化するには SETUP_WIFI_DONGLE=1 を付けて実行する)
#    6. ROS2 ワークスペース kk_ws の作成とリポジトリのクローン
#       (kk_rescue26_pi + .repos の外部依存: ros2_socketcan)
#    7. colcon ビルド
#    8. master control の自動起動(systemd)設定
#    9. ~/.bashrc への ROS2 source 追記
#
...
# 5. 各コンポーネントの依存パッケージ
# =============================================================================
log "5-1. master_control の依存 (psutil)"
sudo apt-get install -y python3-psutil

log "5-2. joy_node_web の依存 (FastAPI / uvicorn / websockets)"
sudo apt-get install -y python3-fastapi python3-uvicorn python3-websockets

log "5-3. camera_publisher の依存 (GStreamer / Python GI / libcamera)"
...
if [ "${SETUP_WIFI_DONGLE}" = "1" ]; then
  log "5-4. USB WiFi ドングルドライバ (RTL8811AU: BUFFALO WI-U2-433 等)"
  ...
  if sudo apt-get install -y --dry-run "${KHDR}" >/dev/null 2>&1; then
    ...
    log "5-4. WiFi ドングルドライバをスキップ: ${KHDR} が入手不可(カーネル更新+再起動後に SETUP_WIFI_DONGLE=1 で再実行)"
  fi
else
  log "5-4. USB WiFi ドングルドライバは既定で無効 (有効化するには SETUP_WIFI_DONGLE=1 を付けて実行)"
fi
...
#    vcstool で取得します。
# =============================================================================
log "6. ワークスペース ${WS} を作成しリポジトリをクローン"
mkdir -p "${WS}/src"
cd "${WS}/src"
...
# =============================================================================
# 8. master control: カメラ/joy_node_web/mic を登録 + 自動起動(systemd)
#    systemd ユニットはこのスクリプトだけが生成します(重複定義を持たない)。
# =============================================================================
log "8-1. programs.json にカメラ / joy_node_web / mic を登録"
...
log "8-2. master-control.service を作成(kk ユーザで port 80 を bind)"
...
# =============================================================================
# 9. ~/.bashrc に ROS2 の source を追記
# =============================================================================
log "9. ~/.bashrc に ROS2 source を追記"
...
echo "  - usb_max_current_enable を新規追記した場合は再起動が必要です。"
