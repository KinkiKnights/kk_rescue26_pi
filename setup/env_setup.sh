...
#    1. パスワード無し sudo の設定
#    2. 2GB スワップ領域の作成
#    3. USB WiFi ドングルドライバ (RTL8811AU) の導入(既定で無効)
#    4. ubuntu-desktop / ros-jazzy-desktop と ROS 開発ツールの導入
#    5. rosdep の初期化・更新
#    6. ~/.bashrc への ROS 2 source 追記
#
...
# =============================================================================
# 4. USB WiFi ドングルドライバ (RTL8811AU) — 既定で無効
...
if [ "${SETUP_WIFI_DONGLE}" = "1" ]; then
  log "4. USB WiFi ドングルドライバ (RTL8811AU: BUFFALO WI-U2-433 等)"
  ...
  if sudo apt-get install -y --dry-run "${KHDR}" >/dev/null 2>&1; then
    ...
    log "4. WiFi ドングルドライバをスキップ: ${KHDR} が入手不可(カーネル更新+再起動後に SETUP_WIFI_DONGLE=1 で再実行)"
  fi
else
  log "4. USB WiFi ドングルドライバは既定で無効 (有効化するには SETUP_WIFI_DONGLE=1 を付けて実行)"
fi
...
# =============================================================================
# 6. ~/.bashrc に ROS 2 の source を追記
# =============================================================================
log "6. ~/.bashrc に ROS 2 source を追記"
